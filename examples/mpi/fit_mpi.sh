...
time mpirun -np 4 ./oral_2cmt_mpi3 sample num_samples=20 num_warmup=150 save_warmup=1 algorithm=hmc stepsize=0.01 adapt delta=0.8 id=$CHAIN  data file=mpi_testgrad-0.R init=0.5 random seed=12 output refresh=10 file=samples-$CHAIN-rect-0.csv

time mpirun -np 4 ./oral_2cmt_mpi3 sample num_samples=20 num_warmup=150 save_warmup=1 algorithm=hmc stepsize=0.01 adapt delta=0.8 id=$CHAIN  data file=mpi_testgrad-1.R init=0.5 random seed=12 output refresh=10 file=samples-$CHAIN-rect-1.csv

time mpirun -np 4 ./oral_2cmt_mpi3 sample num_samples=20 num_warmup=150 save_warmup=1 algorithm=hmc stepsize=0.01 adapt delta=0.8 id=$CHAIN  data file=mpi_testgrad-2.R init=0.5 random seed=12 output refresh=10 file=samples-$CHAIN-rect-2.csv
